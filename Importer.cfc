...
<cffunction name="initInternal" access="public" returntype="any" output="no">
	...
	<cfif
		...
		AND	NOT StructKeyExists(variables,"POIUtility")
	>
		<cftry>
...
<cffunction name="importSpreadsheet" access="public" returntype="boolean" output="no" hint="I return all of the Imports.">
	...
	<cfset var ImportID = 0>
	<cfset var FileBlob = 0>
	...
	<cftry>
		<cfset ImportID = saveImport(argumentCollection=sImport)>
	<cfcatch>
	</cfcatch>
	</cftry>
	...
	<cfif ImportID>
		<cftry>
			<cffile action="readbinary" file="#FilePath#" variable="FileBlob">
			<cfquery datasource="#veriables.DataMgr.getDatasource()#">
			UPDATE	xlsImports
			SET		FileBlob = <cfqueryparam value="#FileBlob#" cfsqltype="CF_SQL_BLOB">
			WHERE	ImportID = #Val(ImportID)#
			</cfquery>
		<cfcatch>
		</cfcatch>
		</cftry>
...
<cffunction name="labelize" access="public" returntype="string" output="false" hint="">
	<cfargument name="string" type="string" required="yes">
	
	<cfset var result = arguments.string>
	<cfset var ii = 0>
	<cfset var letter = "">
	<cfset var letter_prev = "">
	
	<cfloop index="ii" from="#Len(result)-1#" to="2" step="-1">
		<cfset letter = Mid(result,ii,1)>
		<cfset letter_prev = Mid(result,ii-1,1)>
		<!--- Add space if current letter is a capital letter and previous one isn't --->
		<cfif
				( asc(letter) GTE 65 AND asc(letter) LTE 90 )
			AND	( asc(letter_prev) GTE 91 OR asc(letter_prev) LTE 64 )
		>
			<cfset result = Left(result,ii-1) & " " & Mid(result,ii,Len(result)+1-ii)>
		</cfif>
	</cfloop>
...
<cffunction name="makeSampleSpreadsheet" access="public" returntype="string" output="no">
	...
	<cfset var sFields = 0>
	<cfset var ii = 0>
	<cfset var sSheet = StructNew()>
	<cfset var col = "">
	
	<cfif StructKeyExists(variables,"Importee")>
		<cfif NOT Len(Trim(arguments.FieldList))>
			<cfset aFields = variables.Importee.getFieldsArray()>
			<cfloop index="ii" from="1" to="#ArrayLen(aFields)#">
				<cfset arguments.FieldList = ListAppend(arguments.FieldList,aFields[ii].name)>
			</cfloop>
		</cfif>
		
		<!--- Define the columns --->
		<cfset sSheet["ColumnList"] = "">
		<cfset sSheet["ColumnNames"] = "">
		<cfset sSheet["SheetName"] = "Sheet 1">
		
		<cfset sFields = variables.Importee.getFieldsStruct()>
		
		<cfloop list="#arguments.FieldList#" index="col">
			<cfif
					StructKeyExists(sFields,col)
				AND	StructKeyExists(sFields[col],"type")
				AND	isSimpleValue(sFields[col].type)
				AND ListFirst(sFields[col].type,":") NEQ "pk"
				AND	NOT StructKeyExists(sFields[col],"relation")
				AND	NOT ListFindNoCase("DeletionMark,Sorter,DeletionDate,CreationDate,LastUpdatedDate",sFields[col].type)
				AND NOT ListFindNoCase(sSheet["ColumnList"],col)
			>
				<cfset sSheet["ColumnList"] = ListAppend(sSheet["ColumnList"],col)>
				<cfset sSheet["ColumnNames"] = labelize(ListAppend(sSheet["ColumnNames"],col))>
			</cfif>
		</cfloop>
		
		<cfset sSheet.query = QueryNew(sSheet["ColumnList"])>
		
		<!--- Write the file and save the data --->
		<cfset variables.Parent.POIUtility.WriteExcel(
			FilePath=variables.Parent.Manager.FileMgr.getFilePath("#variables.sImporteeMeta.method_Plural#.xls",getFolder("xls,imports,samples")),
			Sheets=sSheet
		)>
	</cfif>
	
	<cfreturn variables.Parent.Manager.FileMgr.getFileURL("#variables.sImporteeMeta.method_Plural#.xls",getFolder("xls,imports,samples"))>
</cffunction>
